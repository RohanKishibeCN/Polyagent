...
import { loadState, saveState } from "../src/engine/state.ts";

const program = new Command()
  .description("Reset persisted engine state for simulation or production")
  .option("--prod", "Reset production state (state/polyagent-prod.json)")
  .option(
    "--active-markets",
...
const statePath = opts.prod
  ? "state/polyagent-prod.json"
  : "state/polyagent.json";
